"""
Strategy 09: MSS + Order Block Entry — Crypto Backtester
==========================================================

Backtests the MSS + OB strategy on cryptocurrency pairs using
Binance data via TradingView.

Usage:
    python crypto_backtester.py
    python crypto_backtester.py --pairs BTCUSDT ETHUSDT --days 30
    python crypto_backtester.py --output custom_results.jsonl
"""
...
# ============================================================================
# DATA FETCHING (reused from strategy 08 pattern)
# ============================================================================

def _resolve_auth_token(session_id: str) -> Optional[str]:
    """
    Convert a TradingView browser sessionid cookie into the websocket
    auth_token that TvDatafeed needs.
    """
    import re
    try:
...
def _subprocess_fetch(symbol, exchange, interval_str, n_bars, result_file,
    ...
    """
    Standalone function (must be top-level for multiprocessing) that
    fetches data from TvDatafeed and writes the result to a temp CSV.
    """
    try:
        if tv_session_token:
            # Token-based auth: create nologin instance, inject token
            tv = TvDatafeed()
        ...
        pass  # result_file won't exist → caller treats as None
...
class CryptoDataFetcher:
    ...
    def _fetch_with_timeout(self, symbol, exchange, tv_interval, n_bars,
        ...
        # Remove the temp file so we can detect if the child wrote it
    ...
    def fetch_ohlcv(
        ...
        # Progressively reduce bar count on retries
        ...
        for attempt in range(1, 5):
            try:
                ...
                df = self._fetch_with_timeout(
                    symbol, "BINANCE", tv_interval, bars, timeout=20,
                )
    ...
    def fetch_multi_timeframe(self, symbol: str) -> Dict[str, pd.DataFrame]:
        ...
        # With auth we can fetch more 5M bars (~17 days vs ~2.7 days)
...
class MSSObBacktester:
    ...
    def backtest_symbol(
        ...
        # --- Optimised: run generate_signal ONCE on full data --
        # instead of 150 per-hour slices, each re-running indicators.
    ...
    def calculate_summary(self, symbol: str, results: List[BacktestResult]) -> BacktestSummary:
        ...
        return BacktestSummary(
            ...
            total_pnl_percent=round(total_pnl, 2), total_pnl_rr=round(total_rr, 2),
            avg_win_percent=round(avg_win, 2), avg_loss_percent=round(avg_loss, 2),
            profit_factor=round(pf, 2), max_drawdown_percent=round(max_dd, 2),
        )
    ...
    def run_backtest(
        ...
        if output_file is None:
            output_file = Path(__file__).parent / "results" / "mss_ob_backtest_signals.jsonl"
        ...
        logger.info(f"Starting MSS+OB backtest on {len(symbols)} crypto pairs...")

        for symbol in tqdm(symbols, desc="Backtesting MSS+OB"):
            try:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            for r in all_results:
                out = {
                    **r.signal,
                    "backtest_exit_time_ist": r.exit_time.astimezone(IST).strftime("%Y-%m-%d %H:%M:%S IST") if r.exit_time else None,
        ...
        return {"results": all_results, "summaries": all_summaries, "output_file": str(output_path)}

    def _print_summary(self, summaries: List[BacktestSummary]):
        print("\n" + "=" * 100)
        print("CRYPTO BACKTEST SUMMARY — MSS + ORDER BLOCK STRATEGY (Path B)")
        print("=" * 100)
        print(f"\n{'Symbol':<12} {'Signals':>8} {'Trades':>7} {'Wins':>5} {'Losses':>7} {'WR%':>7} {'PnL%':>8} {'PnL RR':>8} {'PF':>6}")
        print("-" * 100)
        ...
        for s in summaries:
            print(
                ...
                f"{s.total_pnl_percent:>7.2f}% {s.total_pnl_rr:>7.2f}R "
        ...
        print(f"{'TOTAL':<12} {ts:>8} {tt:>7} {tw:>5} {tl:>7} {wr:>6.1f}% {tp:>7.2f}% {tr:>7.2f}R")
        print("=" * 100)


def main():
    parser = argparse.ArgumentParser(description="Backtest MSS+OB Strategy on Crypto")
    ...
    backtester = MSSObBacktester(strategy=strategy)
